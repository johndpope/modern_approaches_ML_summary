...
# - Without previous y
###############################################################################
trainX = lstm_train_windows.iloc[:, :-1].values
trainX = trainX.reshape(lstm_train_windows.shape[0], 1, trainX.shape[1])

trainY = lstm_train_windows.iloc[:, -1].values

testX  = lstm_test_windows.iloc[:, :-1].values
...
# 1 - CREATE LSTM
model = Sequential()
model.add(LSTM(150, input_shape=(1, trainX.shape[2])))
model.add(Dense(1))
...
# 2 - TRAIN LSTM
t0 = time.time()
history = model.fit(trainX, trainY, epochs=30, batch_size=128, 
